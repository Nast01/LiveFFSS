...
- **`Lane.entry` / `Lane.result` unseen populated.** Both are nullable and safe, but their nesting has never been observed filled in — nor have the payload's `athletes` and `remplacants`, which are deliberately not modelled. Don't guess their shape; get a populated example first.
- **Default lanes are created, never reconciled.** `MeetingRepository.createDefaultLanes` opens a course with 1..n spots. Nothing yet detects a course whose lane count drifted from its round's `spotsPerRace`, because the app cannot create courses at all while `course/submit` is broken.
