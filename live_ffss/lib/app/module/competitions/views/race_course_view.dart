...
class _RaceCourseViewState extends State<RaceCourseView> {
  ...
  Future<void> _openRowMenu(Entry entry, Offset at) async {
    final overlay = Overlay.of(context).context.findRenderObject() as RenderBox;
    final ranked = _ctrl.placeOf(entry) != null;
    final withdrawn = _ctrl.penaltyOf(entry) != null;
    ...
    switch (action) {
      case 'forfeit':
        _ctrl.setPenalty(entry, CoursePenaltyKind.forfeit);
      case 'dq':
        await _askDisqualification(entry);
      case 'unrank':
        _ctrl.remove(entry);
      case 'reinstate':
        _ctrl.clearPenalty(entry);
    }
  }

  Future<void> _askDisqualification(Entry entry) async {
    ...
    field.dispose();
    if (code == null) return;
    _ctrl.setPenalty(entry, CoursePenaltyKind.disqualified, code: code);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Obx(() {
        ...
        if (_ctrl.competitors.isEmpty) {
          return EmptyState(
              icon: Icons.timer_outlined, title: 'no_athletes_found'.tr);
        }
        final rows = _ctrl.orderedCompetitors;
        return Column(
          children: [
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (_, i) => _CompetitorRow(
                  // Keyed on the engagement, not the slot: ranking reorders the
                  // list, and a typed place must follow its own team rather
                  // than stay behind with the row position.
...
/// keystroke: typing « 12 » would otherwise rank the team first in passing.
///
...
class _PlaceField extends StatefulWidget {
  const _PlaceField({required this.entry, required this.place});
...
class _PlaceFieldState extends State<_PlaceField> {
  ...
  void _commit() {
    ...
    Get.find<RaceCourseController>().setPlace(widget.entry, typed);
  }
...
class _CompetitorRow extends GetView<RaceCourseController> {
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      final place = controller.placeOf(entry);
      final penalty = controller.penaltyOf(entry);
      ...
      return Container(
        ...
        child: EntryGroupTile(
          ...
          // The row itself never assigns: brushing one while scrolling ranked
          // whoever it was, which is the handling problem the mode bar exists
          // to fix. Only the trailing control assigns; the long press still
          // opens the menu.
          onLongPress: onMenu,
          // A relay leg carries neither a place nor a menu — the engagement
          // is what gets ranked and what gets penalised.
          onSubstitute: controller.requestSubstitution,
          avatarSize: 28,
          leading: SizedBox(
            width: 40,
            child: manual && penalty == null
                ? _PlaceField(entry: entry, place: place)
                : Text(badge,
                    textAlign: TextAlign.center,
                    style: AppTypography.body.copyWith(
          ...
          trailing: Row(
            ...
            children: [
              ...
              // Automatic mode's one way to rank: an explicit target, not the
              // whole row. Ranked already, it takes the place back.
              if (!manual && penalty == null)
                IconButton(
                  ...
                  color:
                      place == null ? AppColors.primary : AppColors.textMuted,
                  tooltip:
                      place == null ? 'course_assign'.tr : 'course_unassign'.tr,
                ),
              Builder(
